__global__ void TC_INT_Normalization(const int8_t* TC_input, int8_t* TC_output, const int8_t* TC_gamma, const int8_t* TC_beta, 
                                    const int* CC_input, int* CC_output, const int* CC_gamma, const int* CC_beta, 
    ...
    int total_CC_elements = channels * CC_width;
     ...
     if (threadIndex < total_TC_elements) {
        ...
        for (int i = 0; i < CC_width; ++i) {
            int input_idx = channel_start + i;
            int val = CC_input[input_idx];
            sum += val;
            sum_squared += val * val;
        }
        int mean = sum / CC_width;
        int variance = sum_squared / CC_width - mean * mean;

        // Normalize and scale the data
        int normalized_val = ((CC_input[CC_idx] - mean) * 1000000) / (sqrt(variance + 1e-5f) * 1000) + 500000;
        int scaled_val = (CC_gamma[channel_idx] * normalized_val + CC_beta[channel_idx]) / 1000000;
